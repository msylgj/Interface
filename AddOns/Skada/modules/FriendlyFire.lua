...
function mod:Update(win, set)
	...
	for i, player in ipairs(set.players) do
		if player.ffdamagedone > 0 then
			...
			d.valuetext = Skada:FormatNumber(player.ffdamagedone)..(" (%02.2f%%)"):format(player.ffdamagedone / set.ffdamagedone * 100)
			d.id = player.id
...
function spellmod:Update(win, set)
	...
	if player then
		for spellname, spell in pairs(player.ffdamagedonespells) do
			...
			d.id = spellname
			d.spellid = spell.id
			d.valuetext = Skada:FormatNumber(spell.damage)..(" (%02.2f%%)"):format(spell.damage / player.ffdamagedone * 100)
...
function playermod:Update(win, set)
	...
	if player then
		...
		for targetname, target in pairs(player.ffdamagedonetargets) do
			...
			d.icon = nil
			d.id = targetname
			d.valuetext = Skada:FormatNumber(target.damage)..(" (%02.2f%%)"):format(target.damage / player.ffdamagedone * 100)
