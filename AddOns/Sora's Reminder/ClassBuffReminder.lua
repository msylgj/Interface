...
Event:SetScript("OnEvent",function(self, event, unit, ...)
	...
	if event == "PLAYER_LOGIN" or event == "ACTIVE_TALENT_GROUP_CHANGED" then
		ClassBuff = cfg.ClassBuffList[Class][GetSpecialization() or 1]
	...
	if event == "PLAYER_REGEN_DISABLED" then
		...
		for key,value in pairs(ClassBuff) do
			...
			if flag == 0 then
				if Class == "PALADIN" and GetNumShapeshiftForms() > 0 then
					if GetSpecialization() == 2 then
						local Name, _, Icon = GetSpellInfo(25780)
						local index = GetShapeshiftForm()
						if index < 1 or index > GetNumShapeshiftForms() then
							BuffFrame[1].Texture:SetTexture(GetShapeshiftFormInfo(1))
							BuffFrame[1].Texture:SetTexCoord(0.05, 0.95, 0.05, 0.95)
							BuffFrame[1].Text:SetText(format("缺少：%s","圣印"))
							BuffFrame[1]:SetAlpha(1)
						end
						if not UnitAura("player", Name) then	
							BuffFrame[2].Texture:SetTexture(Icon)
							BuffFrame[2].Texture:SetTexCoord(0.05, 0.95, 0.05, 0.95)
							BuffFrame[2].Text:SetText(format("缺少：%s",Name))
							BuffFrame[2]:SetAlpha(1)
						end
					else
						local index = GetShapeshiftForm()
						if index < 1 or index > GetNumShapeshiftForms() then
