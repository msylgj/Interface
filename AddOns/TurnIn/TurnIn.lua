...
function TI_Switch(state)
	if(state=="on") then
		...
		TI_LoadEvents();
		TI_message("\124cff00ff00自动交接任务-开\124r");
	elseif(state=="off") then
		TI_ResetPointers();
		TI_status.state = false;
		TI_UnloadEvents();
		TI_message("\124cffff0000自动交接任务-关\124r");
...
function TI_SlashCmdHandler(cmd)
	...
	if(commands[cmdlist[1]]) then
		...
		TI_message("Turn In 2.1 Help", "--------------", "/ti on - 打开", "/ti off - 关闭", "/ti toggle - 切换", "/ti window - 选项", "/ti recent - 显示最近访问的NPC");
	end
