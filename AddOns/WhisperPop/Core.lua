...
-- BN friends are always saved in battle tags
function addon:GetBNFriendTag(link)
	if type(link) ~= "string" then
		return
	end

	local id = BNet_GetBNetIDAccount(link)
	if not id then
		return
	end

	local _, _, tag  = BNGetFriendInfoByID(tonumber(id) or 0)
	return tag
end
...
addon.DB_DEFAULTS = {
	...
	showRealm = 1,
	foreignOnly = 1,
	buttonScale = { min = 50, max = 200, step = 5, default = 70 },
...
function addon:ProcessChatMsg(name, class, text, inform)
	...
	if class == "BN" then
		name = addon:GetBNFriendTag(name)
		if not name then
			return
		end
	else
...
function addon:CHAT_MSG_BN_WHISPER(text, name)
	self:ProcessChatMsg(name, "BN", text)
end

function addon:CHAT_MSG_BN_WHISPER_INFORM(text, name)
	self:ProcessChatMsg(name, "BN", text, 1)
end
