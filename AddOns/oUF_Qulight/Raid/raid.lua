...
local border = {
    bgFile = [=[Interface\AddOns\oUF_Qulight\Media\white.tga]=],
...
function ns:UpdateHealth(hp)
    ...
    if not Qulight["raidframes"].powerbar then
		if Qulight["unitframes"].party and self.mystyle == "party" then
			hp:SetHeight(Qulight["unitframes"].partysize[1])
			hp:SetWidth(Qulight["unitframes"].partysize[2])
		else
...
local function PostPower(power, unit)
    ...
    if ptype == 'MANA' or (Qulight["unitframes"].party and self.mystyle == "party") then
        power:Show()
        if(Qulight["raidframes"].porientation == "VERTICAL")then
			if Qulight["unitframes"].party and self.mystyle == "party" then
				power:SetWidth(Qulight["unitframes"].partysize[1]*Qulight["raidframes"].powerbarsize)
				self.Health:SetWidth((0.98 - Qulight["raidframes"].powerbarsize)*Qulight["unitframes"].partysize[1])
			else
			...
			if Qulight["unitframes"].party and self.mystyle == "party" then
				power:SetHeight(Qulight["unitframes"].partysize[2]*Qulight["raidframes"].powerbarsize)
				self.Health:SetHeight((0.98 - Qulight["raidframes"].powerbarsize)*Qulight["unitframes"].partysize[2])
			else
...
local styleRaid = function(self)
    ...
    hl:SetTexture([=[Interface\AddOns\oUF_Qulight\Media\white.tga]=])
...
local stylePartyTarget = function(self)
    ...
    self.SpellRange = {
		insideAlpha = 1,
		outsideAlpha = Qulight["raidframes"].outsideRange,}
	self:SetSize(unpack(Qulight["unitframes"].partytargetsize))
	lib.gen_hpbar(self)
...
local function freebHeader(name, group, temp, pet, MT)
    ...
    if Qulight["unitframes"].party then
		...
		'oUF-initialConfigFunction', (initconfig):format(Qulight["raidframes"].width, Qulight["raidframes"].height),
		'showPlayer',Qulight["unitframes"].player,
		'showSolo', Qulight["unitframes"].solo,
		'showParty', true,
		'showRaid', true,
		'xOffset', xoff,
		'yOffset', yoff,
...
oUF:Factory(function(self)
	...
	if Qulight["unitframes"].party then
		...
		'oUF-initialConfigFunction', (initconfig):format(unpack(Qulight["unitframes"].partysize)),
		'showPlayer', Qulight["unitframes"].player,
		'showSolo', Qulight["unitframes"].solo,
		'showParty', true,
		'showRaid', true,
		...
		party:SetPoint(unpack(Qulight["unitframes"].Anchorparty))
